...
const StripeController = {
  stripe: async (req, res) => {
    ...
    const order = await Order.findOne({ user: userId }).sort({ createdAt: -1 }).populate('user').populate({
      path: 'restaurantFK',
      populate: [{ path: 'owner' }]
    });
  ...
  refund: async (req, res) => {
    ...
    try {
      const order = await Order.findOne({ user: userId, statusOrder: 'Canceled' }).sort({ updatedAt: -1 }).populate('user');
      const amountInCents = Math.round(order.totalPrice * 100);
