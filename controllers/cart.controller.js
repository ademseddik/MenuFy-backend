...
getCartTrashByUserweb: async (req, res) => {
  ...
  if (userId && userId !== "undefined" && userId.trim() !== "") {
    try {
      user = new mongoose.Types.ObjectId(userId);  // Utiliser l'ID utilisateur passé en paramètre
    } catch (error) {
      console.log("Invalid userId", error);
      user = guestObjectId;  // Continue en tant qu'invité si l'ID est invalide
    }
  }
...
// },
  ...
  decreaseQuantityProductInCartOrderweb: async (req, res) => {
    ...
    try {
      ...
      // Ensure that the product quantity cannot go below zero
      if (cartOrder.quantityProduct[productIndexOrder] > 1) {
        ...
        // Optionally handle the situation where the quantity would go to zero
