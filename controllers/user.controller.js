...
const sendSMS = async (to, message) => {
  try {
    client.messages.create({
      ...
      to: to, // Numéro de téléphone du destinataire
    });
...
sendSMSWeb: async (req, res) => {
  try {
    ...
    if (!tokenLogin) {
      return res.status(401).json({ message: 'Token de connexion manquant.' });
    }

    let decodeTokenLogin;
    try {
      decodeTokenLogin = jwt_decode(tokenLogin);
    } catch (err) {
    ...
    res.cookie("tokenSend", tokenSend, { expiresIn: "3m", overwrite: true });
    res.json({ tokenSend, message: 'SMS sent successfully and token created.' });
...
verifyCodeWeb: async (req, res) => {
  try {
    ...
    // Vérifiez si le cookie tokenLogin est défini
    if (!tokenProfile) {
    ...
    // Vérifiez si le token est correctement décodé et contient un champ 'id'
    if (!decodeTokenLogin || !decodeTokenLogin.id) {
      return res.status(401).json({ error: "Token de connexion invalide." });
    }
    ...
    // Vérifiez si le code de vérification correspond à celui enregistré dans la base de données pour cet utilisateur
    if (user.verificationCode === verificationCode) {
      // Mettez à jour le statut de vérification de l'utilisateur dans la base de données
      user.isVerified = true;
      await user.save();

      // Code de vérification correct : retournez une réponse avec un statut 200 OK
      return res.status(200).json({ message: "Votre numéro de téléphone a été mis à jour avec succès !" });
    } else {
      // Code de vérification incorrect : retournez une réponse avec un statut 400 Bad Request
...
updatePhoneWeb: async (req, res) => {
  try {
    ...
    if (idUser) {
      await User.updateOne({ "_id": idUser }, { $set: { "phone": phoneNumber } });
    } else {
